{{--
    Assessment Report partial — same format as partials/project-report.blade.php.
    Expects: $reportData (array from AssessmentReportService)
--}}
@php
    ...
    $chartId        = 'reportChart_assess_' . $assessment->id;
@endphp
...
<div class="card report-card mb-4">
    <div class="card-header report-card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
            <h5 class="mb-0 fw-semibold">
                <i class="fa fa-clipboard-list me-2 text-primary"></i>{{ $assessment->name }}
            </h5>
            <small class="text-muted">
                <i class="fa fa-diagram-project me-1"></i>{{ $project->project_name }}
                @if($project->client_name) &nbsp;&mdash;&nbsp; {{ $project->client_name }} @endif
            </small>
        </div>
        <div class="d-flex align-items-center gap-2">
            @if($assessment->submitted_at)
                <span class="badge bg-success px-3 py-2">
                    <i class="fa fa-circle-check me-1"></i>Submitted
                </span>
    ...
    <div class="card-body">
        <div class="row g-3">
            <div class="col-md-4">
                <p class="report-meta-label">Assessment</p>
                <p class="mb-0 small fw-semibold">{{ $assessment->name }}</p>
            </div>
            <div class="col-md-4">
                <p class="report-meta-label">Project</p>
                <p class="mb-0 small">{{ $project->project_name }}</p>
            </div>
            <div class="col-md-4">
                <p class="report-meta-label">Created Date</p>
                <p class="mb-0 small">{{ getDateInFormat($assessment->created_at) }}</p>
            </div>
            @if($assessment->submitted_at)
            <div class="col-md-4">
                <p class="report-meta-label">Submitted On</p>
                <p class="mb-0 small text-success fw-medium">
                    <i class="fa fa-circle-check me-1"></i>{{ $assessment->submitted_at->format('d M Y, h:i A') }}
                </p>
            </div>
            @endif
...
{{-- ── Summary Cards (identical to project-report) ─────────────────────── --}}
...
{{-- ── Chart + Deployment Notes (identical to project-report) ──────────── --}}
<div class="row g-3 mb-4">
    <div class="col-md-5">
...
{{-- ── Category Progress Bars (identical structure to project-report) ───── --}}
@if($categories->count())
<div class="card report-card mb-4">
    <div class="card-header report-card-header">
            ...
            <div class="ps-2">
                ...
                <div class="report-item-row d-flex align-items-start gap-2 py-2 {{ !$loop->last ? 'border-bottom' : '' }}">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        <span class="small {{ $ac->is_checked ? 'text-muted text-decoration-line-through' : '' }}">
                        ...
                        @if($ac->is_checked && $checkedByName)
                            <div class="report-item-meta">
                                ...
                                @if($ac->checked_at)
                                    &nbsp;&bull;&nbsp;<i class="fa fa-calendar-check me-1"></i>{{ \Carbon\Carbon::parse($ac->checked_at)->format('d M Y, h:i A') }}
                                @endif
                            </div>
                        @endif
                    </div>
...
{{-- ── Shared styles (same as project-report partial) ─────────────────── --}}
@once
<style>
...
.report-stat-card { background:#fff; border:1px solid #e9ecef; border-radius:12px; padding:18px 16px 14px; text-align:center; box-shadow:0 1px 4px rgba(0,0,0,.04); }
.report-stat-icon { width:40px; height:40px; border-radius:10px; display:inline-flex; align-items:center; justify-content:center; font-size:1.1rem; margin-bottom:10px; }
...
.report-chart-wrapper { position:relative; width:100%; max-width:220px; margin:0 auto; aspect-ratio:1/1; }
.report-chart-wrapper canvas { width:100% !important; height:100% !important; display:block; }
.report-chart-center { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); text-align:center; pointer-events:none; }
